...
fn draw_ui(frame: &mut Frame, app: &ExploreApp) {
...
fn render_table(frame: &mut Frame, app: &ExploreApp, area: ratatui::layout::Rect) {
    ...
    let visible_height = area.height.saturating_sub(3) as usize; // borders + header row
...
fn build_status_bar(app: &ExploreApp) -> Paragraph<'static> {
    // Build column list with current selection highlighted
