...
- **SVG output embeds real data marks.** Every `--svg`/`--html` chart carries a `<g class="vz-data">` overlay with one `<circle class="vz-point" data-label data-value>` + `<title>` per datum. Downstream SVG parsers that assumed text-rows-only must ignore the `vz-data` group. HTML tooltips read these marks (`label: value`); SVGs without marks keep the old tspan/rect heuristic.
