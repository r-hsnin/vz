...
- ~~`--watch` — ファイル変更を監視して自動再描画~~ ✅ Cycle 131
...
- ~~`oneshot/mod.rs` (36KB) — チャート描画ロジックを分離~~ ✅ Cycle 134 (builders.rs抽出)
- ~~`present/mod.rs` (33KB) — レンダリング部分を切り出し~~ ✅ Cycle 132 (render.rs + chart_loader.rs抽出)
- `explore/mod.rs` (25KB) — イベントハンドリングと描画を分離
- ~~stdin が空のときのエラーメッセージ改善~~ ✅ Cycle 70

## 足回り

- ~~CI に `cargo test` + clippy を追加~~ ✅ Cycle 135
