...
#[test]
fn test_diff_bar_piped_motion_matches_static() {
    let base = vz_binary()
        .args([
            "fixtures/diff/sales_before.csv",
            "fixtures/diff/sales_after.csv",
    ...
    for motion in ["off", "grow", "draw"] {
        ...
        assert_eq!(
            ...
            "diff piped --motion {motion} must match default"
        );
        assert!(!String::from_utf8(out.stdout).unwrap().contains("\x1b[?25"));
    }
}

#[test]
fn test_diff_line_piped_motion_matches_static() {
    let base = vz_binary()
        .args([
            "fixtures/diff/timeseries_before.csv",
            "fixtures/diff/timeseries_after.csv",
    ...
    for motion in ["off", "grow", "draw"] {
        ...
        assert_eq!(
            out.stdout, base.stdout,
            "diff line piped --motion {motion} must match default"
        );
    }
}
